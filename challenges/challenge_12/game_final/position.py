...
Location: Describes the location of the current position of the user on the grid
    locations: A dictionary where the key is a tuple of the x, y coordinate and the value is a tuple of the location
...
class Location(Position):
    ...
    def set_locations(self, locations):
        """
        This function is for setting the locations
        Definitions for each variable are given at the top of this module
        """
        self.locations = locations

    def set_coordinates_and_locations_from_file(self, file):
        """
        This function is for setting the coordinates of the grid from a file
        This function overwrites the one in the position class so that we can also include location name and description
            all in one place with the coordinates
        Definitions for each variable are given at the top of this module
        The file must be in format:
            x coordinate, y coordinate, int bool north direction, int bool east direction, int bool south direction,
                int bool west direction, entry/exit/interior description, location name, location description
        Example:
        0,0,1,0,0,0,Entry
        0,1,1,0,0,0,Interior
        0,2,0,0,0,0,Exit
        ...
        with open(file, 'r') as file:
            for line in file:
                x, y, n, e, s, w, d, ln, ld = line.replace('\n', '').split('|')
                self.coordinates[(int(x), int(y))] = (int(n), int(e), int(s), int(w))
                self.locations[(int(x), int(y))] = (ln, ld)
                if d == 'Entry':
                    self.entry_coord = (int(x), int(y))
                    self.current_position = self.entry_coord
                if d == 'Exit':
    ...
    def look_around(self):
        """
        This functions tells the user what locations are adjacent to them on the grid
        Definitions for each variable are given at the top of this module
        """
        print('Current Position: {}'.format(self.locations[self.current_position][1]))
        print('N: {}'.format(self.locations[(self.current_position[0], self.current_position[1] + 1)][1]))
        print('E: {}'.format(self.locations[(self.current_position[0] + 1, self.current_position[1])][1]))
        print('S: {}'.format(self.locations[(self.current_position[0], self.current_position[1] - 1)][1]))
        print('W: {}'.format(self.locations[(self.current_position[0] - 1, self.current_position[1])][1]))
    ...
    def validate_direction(self):
        ...
        if self.new_position == self.current_position:
            ...
            if self.direction.upper() == 'N':
                self.invalid_direction = self.locations[(x, y + 1)][1]
            elif self.direction.upper() == 'E':
                self.invalid_direction = self.locations[(x + 1, y)][1]
            elif self.direction.upper() == 'S':
                self.invalid_direction = self.locations[(x, y - 1)][1]
            elif self.direction.upper() == 'W':
                self.invalid_direction = self.locations[(x - 1, y)][1]
            print('Cannot go {}: {}'.format(self.direction, self.invalid_direction))
        else:
            self.valid_direction = self.locations[self.new_position][1]
            print('You go {}: {}'.format(self.direction, self.valid_direction))
            self.current_position = self.new_position
